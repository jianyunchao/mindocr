...
Status CrnnInferProcess::Init(CommandParser &options, ModuleInitArgs &initArgs) {
...
Status CrnnInferProcess::DeInit(void) {
    LogInfo << "crnn_infer_process [" << instanceId_ << "]: Deinit success.";
    return Status::OK;
}

Status CrnnInferProcess::ParseConfig(CommandParser &options, AscendBaseModule::ModuleInitArgs &initArgs) {
    ...
    if (staticMethod_) {
        ...
        for (auto &file: files) {
            ...
            } else if (backend_ == BackendType::LITE and Utils::EndsWith(file, ".mindir")) {
