...
Status CrnnPreProcess::Init(CommandParser &options, ModuleInitArgs &initArgs) {
...
Status CrnnPreProcess::ParseConfig(CommandParser &options) {
    ...
    if (!staticMethod_) {
        ...
        recMaxWidth_ = std::ceil(float(recMinWidth_) / FLOAT_WIDTH_RATIO) * INT_WIDTH_RATIO;
        ...
        recMaxWidth_ = std::floor(float(recMaxWidth_) / FLOAT_WIDTH_RATIO) * INT_WIDTH_RATIO;
...
int CrnnPreProcess::GetCrnnMaxWidth(std::vector<cv::Mat> &frames, float maxWHRatio) {
    ...
    for (auto &frame: frames) {
        ...
        int maxWidth = int(maxWHRatio * stdHeight_);
        if (std::ceil(stdHeight_ * ratio) > maxWidth) {
            resizedW = maxWidth;
        } else {
            resizedW = int(std::ceil(stdHeight_ * ratio));
        }
...
Status CrnnPreProcess::Process(std::shared_ptr<void> commonData) {
    ...
    for (unsigned int i: splitIndex) {
        ...
        if (taskType_ == TaskType::DET_CLS_REC or taskType_ == TaskType::DET_REC) {
