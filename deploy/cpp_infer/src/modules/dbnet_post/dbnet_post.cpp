...
Status DbnetPost::DbnetMindXObjectDetectionOutput(std::vector<MxBase::Tensor> &singleResult,
...
Status DbnetPost::DbnetLiteObjectDetectionOutput(std::vector<mindspore::MSTensor> &singleResult,
                                                 std::vector<std::vector<TextObjectInfo>> &textObjInfos,
                                                 const std::vector<ResizedImageInfo> &resizedImageInfos) {
    LogDebug << "DbnetPost start to write results.";
    uint32_t batchSize = singleResult.size();
    for (uint32_t i = 0; i < batchSize; i++) {
        auto probMap = (float *) (singleResult[0].Data().get());
        this->resizedH_ = resizedImageInfos[i].heightResize;
        this->resizedW_ = resizedImageInfos[i].widthResize;
        std::vector<uchar> prob(resizedW_ * resizedH_, ' ');
        std::vector<float> fprob(resizedW_ * resizedH_, 0.f);
...
void DbnetPost::FindContours(std::vector<std::vector<TextObjectInfo>> &textObjInfos,
                             ...
                             std::vector<uchar> &prob, std::vector<float> &fprob) {
