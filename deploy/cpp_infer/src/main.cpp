...
namespace {
    ...
    void ModuleDescGenerator(int deviceNum, std::vector<AscendBaseModule::ModuleDesc> &moduleDesc, TaskType taskType) {
        ...
        if (taskType == TaskType::DET or taskType == TaskType::DET_REC or taskType == TaskType::DET_CLS_REC) {
        ...
        if (taskType == TaskType::CLS or taskType == TaskType::DET_CLS_REC) {
        ...
        if (taskType == TaskType::REC or taskType == TaskType::DET_REC or taskType == TaskType::DET_CLS_REC) {
    ...
    void DescGenerator(CommandParser &options, std::vector<AscendBaseModule::ModuleConnectDesc> &connectDesc,
        ...
        ModuleDescGenerator(device_num, moduleDesc, taskType);

        ModuleConnectDesc(connectDesc, taskType);
    }

    Status InitModuleManager(ModuleManager &moduleManager, CommandParser &options, std::string &aclConfigPath,
    ...
    Status ParseArgs(int argc, const char *argv[], CommandParser &options) {
        ...
        options.AddOption("--device", "", "Device type");
        options.AddOption("--device_type", "", "310p or 310");
        options.AddOption("--device_id", "", "Device id.");
        options.AddOption("--parallel_num", "", "Number of parallel in each stage of pipeline parallelism.");
        ...
        options.AddOption("--rec_height", "", "rec model required height");
        options.AddOption("--rec_min_width", "", "rec model required min width");
        options.AddOption("--rec_max_width", "", "rec model required max width");
    ...
    void
    saveModelGear(std::string modelPath, int32_t &deviceId, const std::string &modelType, BackendType engine) {
    ...
    void
    saveModelBs(std::string modelPath, int32_t &deviceId, const std::string &modelType, BackendType engine) {
        std::vector<std::vector<uint64_t>> dynamicGearInfo;
        if (engine == BackendType::ACL) {
            MxBase::Model model(modelPath, deviceId);
            dynamicGearInfo = model.GetDynamicGearInfo();
...
int main(int argc, const char *argv[]) {
    ...
    if (taskType == TaskType::UNSUPPORTED) {
        ...
        exit(int(Status::COMM_INVALID_PARAM));
    }

    for (int i = 0; i < threadCount; ++i) {
        streamName[i] = "imgSplitFile" + std::to_string(i) + Utils::BoolCast(useCls);
        threadProcess[i] = std::thread(MainProcess, std::ref(options), streamName[i], taskType);
    }
